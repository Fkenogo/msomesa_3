...
import { SparklesIcon, CheckCircleIcon, XCircleIcon } from '../components/icons';
...
const PartReview: React.FC<{ part: QuestionPart; userAnswer: string; questionText: string; pdfSummary?: string }> = ({ part, userAnswer, questionText, pdfSummary }) => {
    const [explanation, setExplanation] = useState<string | null>(null); // Start with null to trigger fetch
    ...
    }, [explanation, part, questionText, pdfSummary]);
    ...
    return (
        <div className="border-t border-gray-200 pt-4 mt-4">
            ...
            <div className="mt-4 text-sm flex items-center gap-4 flex-wrap">
                <details onToggle={(e) => { if ((e.target as HTMLDetailsElement).open) fetchExplanation(); }}>
                    <summary className="cursor-pointer font-semibold text-sky-700 hover:text-sky-800 flex items-center gap-1">
                        <SparklesIcon className="w-4 h-4" /> View Explanation
                    </summary>
                    <div className="prose prose-sm mt-2 p-4 bg-gray-50 rounded-lg whitespace-pre-wrap max-w-none">
                        {isLoadingExplanation ? "Loading AI explanation..." : (explanation || 'Click to load explanation.')}
                    </div>
                </details>
